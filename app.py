"""
RAG de voyage GENOVA, application streamlit
"""
...
llm_chain_for_docs = LLMChain(llm=llm, prompt=prompt)
...
st.set_page_config(
    page_title="Genova - Guide de Voyage IA", page_icon="🌍", layout="wide"
)
...
with st.container():
    ...
    st.markdown(
        '<div class="subtitle">Votre compagnon de route, guide IA des voyageurs curieux</div>',
        unsafe_allow_html=True,
    )
    st.markdown("---")
    ...
    user_input = st.text_input(
        "Où voulez-vous aller ? Que voulez-vous savoir ?", key="temp_input"
    )
    ...
    for question, answer in reversed(st.session_state.chat_history):
        ...
        st.markdown(
            f"<div class='chatbox'><b>Genova :</b> {answer}</div>",
            unsafe_allow_html=True,
        )
